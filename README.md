...
| 层级 | 数据来源 | 可靠性 |
|------|----------|--------|
| **Level 1** | 528+ 个 CalculiX 源码硬编码错误模式 | ✅ 100% 准确 |
...
**AI 诊断规则大幅增强**：
- 规则模式：527 → **528+** 个 CalculiX 源码硬编码错误模式
- 诊断函数：16 → **20** 个检测函数
- 新增检测类别：
  - `FILE_IO_PATTERNS`：文件 I/O 错误（7 个模式）
  - `USER_ELEMENT_PATTERNS`：用户单元/材料错误（4 个模式）
  - `DYNAMICS_PATTERNS`：动力学/模态分析错误（6 个模式）
  - `MPC_LIMIT_PATTERNS`：MPC/约束数量超限（11 个模式）
- 增强 `CONTACT_PATTERNS`：主从面、tied MPC 等（11 个模式）
- 增强 `MATERIAL_PATTERNS`：各向异性、方向等（16 个模式）
...
**AI 诊断规则层增强**：
- Level 1 规则检测：528+ 个 CalculiX 源码硬编码错误模式
- 新增：`MATERIAL_PATTERNS`、`PARAMETER_PATTERNS`、`MPC_LIMIT_PATTERNS`
- 新增：`FILE_IO_PATTERNS`、`USER_ELEMENT_PATTERNS`、`DYNAMICS_PATTERNS`
- 新增：步长停滞检测（连续5个增量 INC TIME 不增大）
- 新增：接触主从面、MPC 数量超限、用户单元、动力学分析等检测
...
**诊断说明：**
- Level 1 规则检测**无需 AI**，基于 528+ 个 CalculiX 源码硬编码模式，0 幻觉
...
│  ├── 528+ 个 CalculiX 源码硬编码模式                   │
│  ├── 收敛性 / 材料缺失 / 输入语法 / 单元质量 / 步长停滞  │
│  ├── 文件 I/O / 用户单元 / MPC 限制 / 动力学分析        │
├─────────────────────────────────────────────────────────┤
...
| MPC 限制 | `increase nmpc_`, `increase nboun_` | 简化模型 |
| 载荷传递 | `RHS only consists of 0.0` | 检查耦合约束 |
| 边界条件 | `zero pivot`, `singular matrix` | 检查约束完整性 |
| 接触问题 | `slave/master surface`, `overclosure` | 检查接触面定义 |
| 文件 I/O | `could not open file` | 检查文件路径 |
| 用户单元 | `umat`, `user element` | 检查子程序链接 |
| 动力学 | `eigenvalue`, `cyclic symmetric` | 检查模态参数 |
